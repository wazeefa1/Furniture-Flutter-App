...
class NonePainter extends BasePainter {
  ...
  void draw(Canvas canvas, double space, double size, double radius) {
    double progress = page - index;
    double secondOffset = index == (widget.count ?? 0) - 1
        ? radius
...
class ScalePainter extends BasePainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0, c = (widget.count ?? 0); i < c; ++i) {
      if (_shouldSkip(i)) {
        continue;
      }
...
class _PageIndicatorState extends State<PageIndicator> {
  ...
  Widget build(BuildContext context) {
    Widget child = new SizedBox(
      width: (widget.count ?? 1) * widget.size + ((widget.count ?? 0) - 1) * widget.space,
      height: widget.size,
